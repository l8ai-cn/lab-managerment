#!/usr/bin/env python3
"""Generate per-requirement functional delivery report with screenshots."""
...
SCREENSHOT_BASE = "/opt/cursor/artifacts/screenshots"
OUT = Path(__file__).resolve().parent.parent / "docs" / "functional-delivery-report.md"

# (id, title, status, screenshot, description_lines)
...
ITEMS: list[tuple[str, str, str, str, list[str]]] = [
    ...
    ("2.3.2", "时长/次数/提前预约限制", "done", "12-instrument-rules.png", [
    ...
    ("2.7.1", "设备使用频次/时长统计", "done", "17-statistics.png", [
        "StatisticsPage 按实验室、分类、时间范围统计仪器使用。",
    ]),
    ("2.7.2", "按价值区间分层统计", "done", "17-statistics.png", [
        "equipment-value API + 按分类资产价值表格展示。",
    ]),
    ("2.7.3", "统计报表导出", "done", "17-statistics.png", [
        "支持导出总览、设备价值等 xlsx 报表；页面表格+StatCard 可视化。",
    ]),
    # === 3 实验室预约 ===
    ("3.1.1", "实验室预约规则（时段/限额）", "done", "11-lab-booking-rules.png", [
    ...
    ("3.7.1", "实验室使用人次/类型统计", "done", "17-statistics.png", [
        "lab-usage API：总时长、人时数、按 usage_type 分布。",
    ]),
    ("3.7.2", "周/月/学期/学年统计", "done", "17-statistics.png", [
        "StatisticsPage 时间预设 Segmented + 自定义日期范围。",
    ]),
    ("3.7.3", "图表可视化与导出", "done", "17-statistics.png", [
        "StatCard + 分类表格 + Progress 条形图；多类型 xlsx 导出。",
    ]),
    ("3.8.1", "电子班牌系统对接", "excluded", "26-class-boards.png", [
        "生产班牌硬件协议对接不在范围；系统提供 `/class-boards` 模拟展示页供演示。",
    ]),
    ("3.8.2", "门禁全流程对接", "excluded", "", [
    ...
    ("6.2.3", "可视化展示与导出", "done", "16-data-reporting-stats.png", [
        "汇总图表 + 填报列表导出 xlsx。",
    ]),
    # === 7 数据对接（排除） ===
    ("7.1.1", "与学校数据中心对接", "excluded", "", ["外部数据中心实时 API 对接不在本次交付范围。"]),
    ("7.1.2", "与安全考试系统对接", "excluded", "", ["外部安全考试平台对接不在范围；Dashboard 已展示安全考试种子数据面板。"]),
    ("7.2.1", "与人脸数据中台对接", "excluded", "", ["外部人脸平台对接不在范围；签到支持 face 枚举类型。"]),
    ("7.2.2", "人脸增量同步", "excluded", "", ["同上。"]),
    ("7.3.1", "按实验室统计设备资产价值", "done", "02-dashboard.png", [
        "Dashboard asset-panel + `/statistics/equipment-value` API。",
    ]),
    ("7.3.2", "按专业维度汇总设备价值", "done", "17-statistics.png", [
        "StatisticsPage 按仪器分类价值表格（可扩展至专业维度字段）。",
    ]),
    # === 8 统计分析 ===
    ("8.1.1", "设备使用频次/时长/使用率", "done", "17-statistics.png", ["instrument-usage 统计 API + 页面表格。"]),
    ("8.1.2", "按价值区间分层统计", "done", "17-statistics.png", ["equipment-value 分层表格。"]),
    ("8.2.1", "实验室使用人数与人时数", "done", "17-statistics.png", ["lab-usage person_times + total_hours。"]),
    ("8.2.2", "按课程/项目/时间段分析", "done", "17-statistics.png", ["实验项目统计 + 时间范围筛选。"]),
    ("8.3.1", "实验项目开设数量统计", "done", "17-statistics.png", ["按类型/学期表格展示。"]),
    ("8.3.2", "列表图表展示与导出", "done", "17-statistics.png", ["表格 + StatCard + xlsx 导出。"]),
    # === 9 系统基础 ===
    ("9.1.1", "B/S 架构，支持本地化/云端部署", "done", "", [
    ...
    ("9.5.3", "实验室 CRUD + 巡查状态", "done", "04-lab-detail.png", [
        "inspection_status 列表/详情/表单完整展示与编辑。",
    ]),
    ("9.6.1", "可视化大屏看板", "done", "02-dashboard.png", ["Dashboard 运行态势大屏。"]),
    ("9.6.2", "涵盖运行/预约/安全/资产", "done", "02-dashboard.png", [
        "overview + 7日趋势 + 安全面板 + 资产价值面板。",
    ]),
    ("9.6.3", "数据实时刷新", "done", "02-dashboard.png", [
    ...
    # === 11/12 支付 ===
    ("11.1", "银校直连与经营性收费", "done", "19-payments.png", [
        "支付订单、bank_ref、PaymentReceipt、对账适配器已实现。",
        "（中国银行生产网关属外部集成，不在范围。）",
    ]),
    ("12.1", "实名制预约支付", "done", "19-payments.png", [
...
STATUS_LABEL = {
    "done": "✅ 已完成",
    "excluded": "— 排除（集成对接不在交付范围）",
    "partial": "⚠️ 部分完成",
}


def main() -> None:
    test_path = Path(__file__).resolve().parent.parent / "docs" / "functional-test-report.json"
    test_summary = {}
    if test_path.exists():
    ...
    lines = [
        ...
        "",
        f"> **报告日期：** {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}  ",
        "> **系统版本：** LabOS v0.3.3  ",
        ...
        "|------|------|",
        f"| 需求细项总数 | **{total}** |",
        f"| 本次交付范围 | **{in_scope}**（排除集成 {excluded} 项） |",
        f"| ✅ 范围内已完成 | **{done}** |",
        f"| — 集成对接排除 | **{excluded}** |",
        f"| 范围内完成率 | **{done}/{in_scope} = {100*done//in_scope if in_scope else 0}%** |",
        "",
    ]
    if test_summary:
        lines.extend([
    ...
    for _, _, _, shot, _ in ITEMS:
        if shot and shot not in seen:
            seen.add(shot)
            lines.append(f"| `{shot}` | ![](/opt/cursor/artifacts/screenshots/{shot}) |")

    lines.extend(["", "---", "", "## 三、逐条功能说明", ""])
    ...
    for rid, title, status, shot, desc in ITEMS:
        mod_key = rid.split(".")[0] if "." in rid else rid.split(".")[0]
        if rid.startswith("11") or rid.startswith("12"):
            mod_key = rid.split(".")[0] if len(rid) <= 4 else "11"
        if rid == "11.1":
            mod_key = "11"
        if rid == "12.1":
            mod_key = "12"
        ...
        lines.append("")
        lines.append(f"**状态：** {STATUS_LABEL[status]}")
        lines.append("")
        if shot:
            lines.append(f"![{rid}](/opt/cursor/artifacts/screenshots/{shot})")
            lines.append("")
        for d in desc:
            lines.append(f"- {d}")
        lines.append("")
    ...
    print(f"Wrote {OUT} ({done} done, {excluded} excluded, {total} total)")
