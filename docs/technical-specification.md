...
| 变更项 | 说明 |
|--------|------|
| **P0** | 预约规则引擎：`create_booking` 执行开放时段/角色/日限额/时长/提前时间/类型差异/校外规则校验 |
| **P0** | 知识库 FTS 中文分词（CJK 字符级索引）+ UUID 匹配修复 |
| **P1** | 种子数据自动同步门禁/班牌设备 |
| **P1** | Text-to-SQL 支持 `question` 自然语言参数（规则模板转换） |
| **P1** | 种子数据预置仪器预约规则（15 台） |
